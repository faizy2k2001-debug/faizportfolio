...
async function startServer() {
  ...
  // --- SECURE ADMIN AUTHENTICATION WITH lockout ---
  app.post("/api/admin/login", (req, res) => {
    ...
    const attempt = FAILED_LOGIN_ATTEMPTS.get(userKey) || { attempts: 0 };

    // Check if account is currently locked out
    if (attempt.lockUntil && attempt.lockUntil > Date.now()) {
      const secRemaining = Math.ceil((attempt.lockUntil - Date.now()) / 1000);
      const minRemaining = Math.ceil(secRemaining / 60);
      return res.status(403).json({
        error: `Security Lockout active: Too many failed attempts. Try again in ${minRemaining} minute(s) (${secRemaining}s).`
      });
    }

    const db = readDB();
    const creds = db.credentials || { username: "admin", passwordHash: "", salt: "" };

    // Standardized secure comparison using crypto pbkdf2 with salt
    const calculatedHash = crypto.pbkdf2Sync(password, creds.salt, 1000, 64, "sha512").toString("hex");

    if (userKey === creds.username.toLowerCase().trim() && calculatedHash === creds.passwordHash) {
      // Successful login -> reset failed attempts
      FAILED_LOGIN_ATTEMPTS.delete(userKey);

      // Successful credentials match -> Create dynamic token
      const token = crypto.randomBytes(32).toString("hex");
      const expiresAt = Date.now() + 24 * 60 * 60 * 1000; // 24-hour lifetime

      SESSION_TOKENS.set(token, { username: creds.username, expiresAt });
      return res.json({ token, username: creds.username });
    }

    // Increment failed login count
    attempt.attempts += 1;
    if (attempt.attempts >= 5) {
      attempt.lockUntil = Date.now() + 15 * 60 * 1000; // 15-minute lock
      FAILED_LOGIN_ATTEMPTS.set(userKey, attempt);
      return res.status(403).json({
        error: "Security Lockout: Too many failed login attempts. This account has been locked for 15 minutes."
      });
    }

    FAILED_LOGIN_ATTEMPTS.set(userKey, attempt);
    return res.status(401).json({
      error: `Access Denied: Invalid credentials. Attempt ${attempt.attempts} of 5 before safety lockout.`
    });
  });
